...
func (s *Store) Put(e *loggregator_v2.Envelope, index string) {
	...
	if elapsedLock > time.Millisecond {
		log.Printf("Store.Put() took %s to acquire mutex", elapsedLock.String())
	}
	...
	elapsedLock = time.Now().Sub(startLock)
	if elapsedLock > time.Millisecond {
		log.Printf("Store.Put() returned after %s", elapsedLock.String())
	}
}
...
func (s *Store) Get(
	...
	elapsedLock := time.Now().Sub(startLock)
	if elapsedLock > time.Millisecond {
		log.Printf("Store.Get() took %s to acquire mutex", elapsedLock.String())
	}
	...
	elapsedLock = time.Now().Sub(startLock)
	if elapsedLock > time.Millisecond {
		log.Printf("Store.Get() returned after %s", elapsedLock.String())
	}
